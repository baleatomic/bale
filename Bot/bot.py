...
botname = 'BaleBot' ##isi idnya
password = 'BaleBot1234' ##isi paswordnya
...
#Well, you can actually take a little time to look at it and learn something
  ...
  def onMessage(self, room, user, message):
   try:
    ...
    if "Bale" == message.body:
     if self.getAccess(user) >= 1 or room.getLevel(user) > 0 and not user.name in blacklist: 
      room.message (random.choice(["yes "+sntonick(user.name)+ " can i help you ? "]),True)
    ...
    ##check access and ignore
      ...
      if cmd == "cmds":
        if user.name in owners and not user.name in admin and not user.name in archknight and not user.name in whitelist:
          room.message("<br/>"+user.name+" Rank 4 [Owner] "+"<br/>"+" Perintah[ > ] :<br/>wl , gs(Google search) , yt(Youtube) , df(define) , udf(undefine) , fax , bc , sn(sendnote) , rn(readnote) , join , leave , mydict, nick , staff ,  setnick , mynick , seenick , profile , rank , myrank , ranker ,",True)
        if user.name in admin and not user.name in owners and not user.name in archknight and not user.name in whitelist:
          room.message("<br/>"+user.name+" Rank 3 [Admin] "+"<br/>"+" Perintah[ > ] :<br/>wl , gs(Google search) , yt(Youtube) , df(define) , udf(undefine) , fax , bc , sn(sendnote) , rn(readnote) , join , leave , mydict, nick , staff , setnick , mynick , seenick , profile , rank , myrank , ranker ,",True) 
        if user.name in archknight and not user.name in owners and not user.name in admin and not user.name in whitelist:
          room.message("<br/>"+user.name+" Rank 2 [Archknight] "+"<br/>"+" Perintah[ > ] :<br/>wl , gs(Google search) , yt(Youtube) , df(define) , udf(undefine) , fax , bc , sn(sendnote) , rn(readnote) , join , leave , mydict, nick , staff , setnick , mynick , seenick , profile , rank , myrank , ranker ,",True)
        if user.name in whitelist and not user.name in owners and not user.name in admin and not user.name in archknight:
          room.message("<br/>"+user.name+" Rank 1 [Whitelist] "+"<br/>"+" Perintah[ > ] :<br/>wl , gs(Google search) , yt(Youtube) , df(define) , udf(undefine) , fax , bc , sn(sendnote) , rn(readnote) , join , mydict, nick , staff , setnick , mynick , seenick , profile , rank , myrank , ranker ,",True)
      #
      if cmd == "cmds2":
        room.message("<br/>"+user.name+" "+"<br/>"+" Perintah[ > ] :<br/>wl , gs(Google search) , yt(Youtube) , df(define) , udf(undefine) , fax , bc , sn(sendnote) , rn(readnote) , join , mydict, nick , staff ,  setnick , mynick , seenick , profile , rank , myrank , ranker ,",True)
